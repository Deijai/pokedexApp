...
export const PokemonScreen = ({ route }: Props) => {
  ...
  return (
    ...
    <View style={ styles.headerContainer }>
      ...
      {/* Pokeball */ }
      <Image source={ pokeballmage } style={ styles.pokeball } />
    ...
    <View
      ...
      { pokemon.types.map( type => (
        <Chip
          ...
          mode="outlined"
          selectedColor='white'
          style={ { marginLeft: 10, backgroundColor: pokemon.color, opacity: 0.9 } }>
          { type }
        </Chip>
      ) ) }
    </View>
    ...
    />
     ...
     <Text style={styles.subTitle}>Abilities</Text>
      <FlatList
        ...
        showsHorizontalScrollIndicator={false}
        renderItem={({item}) => (
          <Chip selectedColor="white"  style={ { marginLeft: 10, backgroundColor: pokemon.color, opacity: 0.9 } }>{Formatter.capitalize(item)}</Chip>
        )}
      />
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          <View style={styles.statsContainer}>
            <Text style={{flex: 1, color: 'white'}}>
              {Formatter.capitalize(item.move.name)}
            </Text>
            <Text style={{color: 'white'}}>lvl {item.move.url}</Text>
          </View>
        )}
      />
